...
class OoriginPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `origin`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM origin WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO origin (date, title, link, content)"
